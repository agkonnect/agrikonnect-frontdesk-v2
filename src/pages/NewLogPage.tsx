...
// Small popup showing previous visits for a phone number
function PhoneHistoryModal({ rows, phone, onClose }: { rows: PhoneHistoryRow[]; phone: string; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(9,9,11,.6)', backdropFilter: 'blur(3px)' }}
      />
      {/* Modal */}
      <div style={{
        ...
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--dim)', marginTop: 1 }}>
              {phone} · {rows.length} previous contact{rows.length !== 1 ? 's' : ''}
            </div>
          </div>
          <button
            onClick={onClose}
            style={{ width: 26, height: 26, borderRadius: 6, background: 'transparent', border: '1px solid var(--border)', color: 'var(--dim)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }}
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {rows.map((r, i) => (
            <div key={r.id} style={{
              padding: '12px 18px',
              borderBottom: i < rows.length - 1 ? '1px solid rgba(63,63,70,.4)' : 'none',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)' }}>
                    {INTENT_LABELS[r.intent as keyof typeof INTENT_LABELS] ?? r.intent}
                  </span>
                  {r.crop && (
                    <span style={{ fontSize: 11, color: 'var(--dim)' }}>
                      · {r.crop}{r.quantity ? `, ${r.quantity}` : ''}
                    </span>
                  )}
                  {r.region && (
                    <span style={{ fontSize: 11, color: 'var(--dim)' }}>· {r.region}</span>
                  )}
                </div>
                {r.notes && (
              ...
              {/* Outcome badge */}
              <span className={outcomeBadge(r.outcome as Parameters<typeof outcomeBadge>[0])} style={{ flexShrink: 0 }}>
                {OUTCOME_LABELS[r.outcome as keyof typeof OUTCOME_LABELS] ?? r.outcome}
              </span>
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <label className="form-label">
        {label}{required && <span style={{ color: 'var(--emerald)', marginLeft: 2 }}>*</span>}
      </label>
      {children}
    </div>
  )
    ...
    toast(
      (t) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <button onClick={() => toast.dismiss(t.id)} style={{ alignSelf: 'flex-end', fontSize: 10, color: '#6b7280', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            Dismiss
          </button>
        </div>
      ),
    ...
    <div style={{ padding: '0 28px' }}>
      <form onSubmit={handleSubmit}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#fbbf24', fontWeight: 600 }}>
                    {phoneHistory.length} previous contact{phoneHistory.length !== 1 ? 's' : ''} found for this number
                  </span>
                </div>
                <button
                  type="button"
                  ...
                  onMouseOver={e => (e.currentTarget.style.background = 'rgba(245,158,11,.25)')}
        ...
        <div className="section-block" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: form.followup_needed ? 14 : 0 }}>
            <button
              type="button"
              role="switch"
              aria-checked={form.followup_needed}
              onClick={() => set('followup_needed', !form.followup_needed)}
              style={{
                width: 36, height: 20,
                background: form.followup_needed ? 'var(--emerald)' : 'var(--surface2)',
                borderRadius: 99, position: 'relative', cursor: 'pointer',
                transition: 'background .2s', flexShrink: 0,
                border: `1px solid ${form.followup_needed ? 'var(--emerald)' : 'var(--border)'}`,
                outline: 'none',
              }}
            >
              <span style={{
                ...
                transition: 'transform .2s', boxShadow: '0 1px 3px rgba(0,0,0,.3)',
                transform: form.followup_needed ? 'translateX(16px)' : 'translateX(0)', display: 'block',
              }} />
            </button>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginBottom: 28 }}>
          <button type="button" className="btn btn-ghost" onClick={() => { setForm(DEFAULT_FORM_VALUES); setPhoneHistory([]) }} disabled={submitting}>
            Reset
          </button>
