...
/** Aggregate crop counts from fetched logs, sorted highest first */
function topCrops(logs: DailyLogWithProfiles[]): { crop: string; count: number }[] {
  ...
  for (const log of logs) {
    ...
    if (!key) continue
    // Normalise to title-case for grouping ("maize" + "Maize" → same bucket)
...
export default function TodayPage({ isAdmin }: Props) {
  const { logs, stats, loading, error, fetchTodayLogs, updateLog } = useLogs()
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, padding: '0 28px', marginBottom: 20 }}>
        <KpiCard label="Total Logs Today"     value={stats.total}            />
        ...
        <KpiCard label="Follow-ups Due Today" value={stats.followupsDueToday} valueColor="#fbbf24" labelColor="var(--amber)" />
      </div>
        ...
        <div style={{ padding: '0 28px', marginBottom: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '10px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--dim)' }}>— logged today</span>
            </div>
                ...
                return (
                  <div key={crop} style={{
                    ...
                    <span style={{
                      ...
                      color: c.text, lineHeight: 1, marginLeft: 2,
                    }}>{count}</span>
                  </div>
                )
              })}
        ...
        <div className="card">
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="data-table">
              <thead>
                <tr>
                  <th>Time</th><th>Type</th><th>Channel</th><th>Phone</th>
              ...
              <tbody>
                ...
                {!loading && logs.length === 0 && (
                  <tr><td colSpan={9} style={{ padding: '40px 14px', textAlign: 'center', color: 'var(--faint)' }}>No logs found.</td></tr>
                )}
                {logs.map(log => {
                  const hasMatches = matchInList(log, logs).length > 0
                  return (
                  <tr key={log.id} onClick={() => setSelectedLog(log)}>
                    <td className="mono">{format(new Date(log.created_at), 'HH:mm')}</td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <span className={contactTypeBadge(log.contact_type)}>{CONTACT_TYPE_LABELS[log.contact_type]}</span>
                        {hasMatches && (
                          <span title="Potential match found" style={{ fontSize: 11, cursor: 'default' }}>⚡</span>
                        )}
                      </div>
                    </td>
